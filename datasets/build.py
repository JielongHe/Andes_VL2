...
def build_dataloader(args, tranforms=None):
    ...
    if args.training:
        ...
        ds_train = dataset.train_select
        train_img_set = ImageDataset(ds_train['image_pids'], ds_train['img_paths'],
                                    test_transforms)
        train_txt_set = TextDataset(ds_train['caption_pids'],
                                   ds_train['captions'],
                                   text_length=args.text_length)

        train_img_loader = DataLoader(train_img_set,
                                     batch_size=args.test_batch_size,
                                     shuffle=False,
                                     num_workers=num_workers)
        train_txt_loader = DataLoader(train_txt_set,
                                     batch_size=args.test_batch_size,
                                     shuffle=False,
                                     num_workers=num_workers)

        return train_img_loader, train_txt_loader, test_img_loader, test_txt_loader, num_classes
